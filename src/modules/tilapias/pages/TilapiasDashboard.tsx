...
import { useTilapiasStore, deleteLote } from '../store'
import { Link } from 'react-router-dom'
import { Fish, DollarSign, TrendingUp, Package, Eye, Plus, Trash2, Calendar, Wallet } from 'lucide-react'
import { formatMoney, formatNumber, getMonthKey, getMesLabel } from '../utils'

export default function TilapiasDashboard() {
  const { state, getBajasByLote, getVentasByLote, getGastosByLote } = useTilapiasStore()
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Tilapias</h1>
        <div className="flex gap-2">
          <Link to="/tilapias/nuevo" className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
          ...
          {lotesFiltrados.map((lote) => {
            ...
            return (
              <div key={lote.id} className={`rounded-xl shadow-sm border p-4 transition-all ${
                isActive
                  ? 'bg-white dark:bg-slate-800 border-l-4 border-l-emerald-500 border-slate-200 dark:border-slate-700 hover:shadow-md'
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={`text-xs ${isActive ? 'text-slate-500 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500'}`}>Inicio: {lote.fechaInicio}</p>
                  </div>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <Link to={`/tilapias/${lote.id}`} className="flex items-center gap-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"><Eye size={16} />Detalle</Link>
                </div>
              </div>
            )
          })}
