...
import { useVacunoStore, deleteLote } from '../store'
import { Link } from 'react-router-dom'
import { DollarSign, TrendingUp, Package, Eye, Plus, Trash2, Calendar, Wallet, Beef } from 'lucide-react'
import { formatMoney, formatNumber, getMonthKey, getMesLabel } from '../utils'

export default function VacunoDashboard() {
  const { state, getVentasByLote, getBajasByLote, getGastosByLote } = useVacunoStore()
  ...
  const meses: Record<string, { ventas: number; costos: number; gastos: number; bajas: number }> = {}
  state.ventas.forEach((v) => {
    const key = getMonthKey(v.fecha)
    if (!meses[key]) meses[key] = { ventas: 0, costos: 0, gastos: 0, bajas: 0 }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Ganado Vacuno</h1>
        <div className="flex gap-2">
          <Link to="/vacuno/nuevo" className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 text-blue-600 mb-1"><Package size={20} /><span className="text-xs font-medium text-slate-500 dark:text-slate-400">Lotes</span></div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 text-red-500 mb-1"><TrendingUp size={20} /><span className="text-xs font-medium text-slate-500 dark:text-slate-400">Bajas</span></div>
          <p className="text-xl lg:text-2xl font-bold truncate text-red-500 dark:text-red-400">{formatNumber(totalBajas)}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 text-amber-600 mb-1"><Wallet size={20} /><span className="text-xs font-medium text-slate-500 dark:text-slate-400">Fiados</span></div>
          <p className="text-xl lg:text-2xl font-bold truncate text-amber-600 dark:text-amber-400">{fiados.length}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 border border-slate-200 dark:border-slate-700">
      ...
      {lotesFiltrados.length === 0 ? (
        <div className="text-center py-16 text-slate-400 dark:text-slate-500">
          <Beef size={48} className="mx-auto mb-3 opacity-50" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {lotesFiltrados.map((lote) => {
            ...
            const vendidos = ventasL.reduce((s, v) => s + v.cantidad, 0)
            const vivos = lote.cantidadInicial - bajas - vendidos
            const isActive = lote.activo
            return (
              <div key={lote.id} className={`rounded-xl shadow-sm border p-4 transition-all ${
                isActive
                  ? 'bg-white dark:bg-slate-800 border-l-4 border-l-amber-700 border-slate-200 dark:border-slate-700 hover:shadow-md'
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={`text-xs truncate ${isActive ? 'text-slate-500 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500'}`}>{lote.raza} · Inicio: {lote.fechaInicio}</p>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-3 text-center text-sm mb-3">
                  ...
                  <div><p className="text-slate-400 dark:text-slate-500 text-xs truncate">Vendidos</p><p className="font-semibold text-blue-600 truncate">{formatNumber(vendidos)}</p></div>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <Link to={`/vacuno/${lote.id}`} className="flex items-center gap-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"><Eye size={16} />Detalle</Link>
                    <Link to={`/vacuno/${lote.id}`} className="text-xs text-rose-600 hover:text-rose-800 dark:text-rose-400">+ Gasto</Link>
                  </div>
                </div>
              </div>
            )
