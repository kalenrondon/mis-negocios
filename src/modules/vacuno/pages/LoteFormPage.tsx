...
export default function LoteFormPage() {
  ...
  return (
    <div className="max-w-lg mx-auto">
      <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-6">Nuevo Lote de Vacunos</h1>
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 space-y-4">
        <div>
          ...
          <input type="text" required value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ej: Lote 1 - Feedlot" />
        </div>
        <div>
        ...
        <div>
          ...
          <NumberInput value={cantidad} onChange={setCantidad} required placeholder="Ej: 50" />
        </div>
        <div>
        ...
        <div>
          ...
          <MoneyInput value={costo} onChange={setCosto} placeholder="Ej: 50.000" required />
        </div>
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/vacuno')} className="flex-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 py-2 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors font-medium">Cancelar</button>
        </div>
      </form>
    </div>
  )
